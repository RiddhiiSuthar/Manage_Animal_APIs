...
from schemas import city, cat, dog
...
def create_cats_in_city(city_name,  cat: cat, db: Session = Depends(get_db)):
    ...
    db_cat = db.query(models.Cat).filter_by(favorite_fish=cat.dict().get("favorite_fish")).first()
...
def update_cat_in_city(city_name, cat_id: UUID, cat: cat, db: Session = Depends(get_db)):
    city = db.query(models.City).filter(models.City.name == city_name).first()
    if city is None:
        raise HTTPException(status_code=404, detail="City not found")
...
def create_dogs_in_city(city_name,  dog: dog, db: Session = Depends(get_db)):
    ...
    db_dog = db.query(models.Dog).filter_by(bark_decibels=dog.dict().get("bark_decibels")).first()
    if db_dog:
        for key, value in dog.dict().items():
...
def update_dog_in_city(city_name, dog_id: UUID, dog: dog, db: Session = Depends(get_db)):
    city = db.query(models.City).filter(models.City.name == city_name).first()
    if city is None:
        raise HTTPException(status_code=404, detail="City not found")
...
def cat_statistics(db: Session = Depends(get_db)):
    stats = db.query(models.City.id, models.City.name, func.count(models.Cat.id).label("total")).outerjoin(models.Cat).group_by(models.City.id).all()
    return [{"id": city_id, "name": name, "total": total} for city_id, name, total in stats]
...
def dog_statistics(db: Session = Depends(get_db)):
    ...
    ).outerjoin(models.Dog).group_by(
        models.City.id,  
        models.Animal.breed
    ).all()

    return [{"id": city_id, "name": name, "animal_breed": breed, "decibels": decibels} for city_id, name, breed, decibels in stats]